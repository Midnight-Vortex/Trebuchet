...
public class Operations : IDisposable
{
    ...
    public async Task<bool> OnBoardingApplyConanManagement()
    {
        ...
        // Enhanced + ManageClient: Hybrid Saved (real Saved/ExtractedMods). Never create whole Saved→GameSaved.
        if (_setup.IsEnhanced)
            return await OnBoardingApplyEnhancedHybridManagement(clientDirectory, savedDir);
    ...
    /// <summary>
    /// Enhanced client Saved management: keep a real Saved (and ExtractedMods) on the game drive.
    /// Launch builds Hybrid child junctions; install/repair must never recreate whole Saved→GameSaved.
    /// </summary>
    private async Task<bool> OnBoardingApplyEnhancedHybridManagement(string clientDirectory, string savedDir)
    {
        if (_setup.Config.ManageClient)
        {
            if (!Directory.Exists(savedDir))
            {
                _logger.LogWarning(@"Saved Directory does not exist (Enhanced Hybrid)");
                if (!await OnBoardingElevationRequest(clientDirectory, Resources.OnBoardingManageConanUac)) return false;
                EnsureEmptyHybridSavedShell(savedDir);
                return true;
            }

            if (_osSpecific.IsSymbolicLink(savedDir))
            {
                _logger.LogWarning(@"Whole-Saved junction found; converting to Hybrid real Saved");
                if (!await OnBoardingElevationRequest(clientDirectory, Resources.OnBoardingManageConanUac)) return false;
                _osSpecific.RemoveSymbolicLink(savedDir);
                EnsureEmptyHybridSavedShell(savedDir);
                return true;
            ...
            var configPath = Path.Combine(savedDir, Constants.FolderConfig);
            if (_osSpecific.IsSymbolicLink(configPath))
            {
                _logger.LogInformation(@"Hybrid Saved already active");
                return true;
            }

            // Empty / Hybrid-ready shell left for launch — do not re-prompt every startup.
            if (!Directory.Exists(configPath)
                && !Directory.Exists(Path.Combine(savedDir, Constants.FolderSaveGames)))
            {
                Directory.CreateDirectory(Path.Combine(savedDir, Constants.FolderExtractedMods));
                return true;
            }

            _logger.LogWarning(@"Real Saved with game data; importing into profile for Hybrid");
            if (!await OnBoardingElevationRequest(clientDirectory, Resources.OnBoardingManageConanUac)) return false;
            var saveName = await OnBoardingChooseClientSaveName();
            _logger.LogInformation(@"Copying game save into trebuchet {saveName}", saveName);
            await Tools.DeepCopyAsync(
                savedDir,
                _appFiles.Client.GetDirectory(_appFiles.Client.Ref(saveName)),
        ...
        // ManageClient off: materialize profile back into a real unmanaged Saved.
        if (!Directory.Exists(savedDir))
        {
            if (!_appFiles.Client.GetList().Any())
                return true;
        ...
        if (_osSpecific.IsSymbolicLink(savedDir))
        {
            _logger.LogWarning(@"Whole-Saved junction found, but does not manage (Enhanced)");
            if (!_appFiles.Client.GetList().Any())
                return true;
            if (!await OnBoardingElevationRequest(clientDirectory, Resources.OnBoardingManageConanUac)) return false;
            var saveName = await OnBoardingChooseClientSave();
        ...
        if (IsHybridClientSavedLayout(savedDir))
        {
            _logger.LogWarning(@"Hybrid Saved found, but does not manage; materializing profile into game Saved");
            if (!_appFiles.Client.GetList().Any())
                return true;
            if (!await OnBoardingElevationRequest(clientDirectory, Resources.OnBoardingManageConanUac)) return false;
            var saveName = await OnBoardingChooseClientSave();
            _logger.LogInformation(@"Materializing Hybrid Saved from profile {saveName}", saveName);
            await MaterializeHybridSavedFromProfile(
                savedDir,
                _appFiles.Client.GetDirectory(_appFiles.Client.Ref(saveName)));
            return true;
        }
    ...
    private static void EnsureEmptyHybridSavedShell(string savedDir)
    {
        Directory.CreateDirectory(savedDir);
        Directory.CreateDirectory(Path.Combine(savedDir, Constants.FolderExtractedMods));
    }

    private bool IsHybridClientSavedLayout(string savedDir)
    {
        if (_osSpecific.IsSymbolicLink(savedDir) || !Directory.Exists(savedDir))
            return false;
        return Constants.HybridSavedLinkedDirectories.Any(child =>
            _osSpecific.IsSymbolicLink(Path.Combine(savedDir, child)));
    }

    private async Task MaterializeHybridSavedFromProfile(string savedDir, string profileFolder)
    {
        foreach (var child in Constants.HybridSavedLinkedDirectories)
        {
            var linkPath = Path.Combine(savedDir, child);
            var profileChild = Path.Combine(profileFolder, child);
            if (_osSpecific.IsSymbolicLink(linkPath))
                _osSpecific.RemoveSymbolicLink(linkPath);
            else if (Directory.Exists(linkPath))
                await OnBoardingSafeIO(() => Directory.Delete(linkPath, true), linkPath);

            if (Directory.Exists(profileChild))
            {
                Directory.CreateDirectory(linkPath);
                await Tools.DeepCopyAsync(profileChild, linkPath, CancellationToken.None);
            }
        }

        var profileExtracted = Path.Combine(profileFolder, Constants.FolderExtractedMods);
        var savedExtracted = Path.Combine(savedDir, Constants.FolderExtractedMods);
        if (_osSpecific.IsSymbolicLink(savedExtracted))
            _osSpecific.RemoveSymbolicLink(savedExtracted);
        Directory.CreateDirectory(savedExtracted);
        if (Directory.Exists(profileExtracted))
            await Tools.DeepCopyAsync(profileExtracted, savedExtracted, CancellationToken.None);
        ...
        foreach (var file in Directory.EnumerateFiles(profileFolder))
        {
            ...
            if (string.Equals(name, Constants.FileProfileConfig, StringComparison.OrdinalIgnoreCase))
                continue;
            File.Copy(file, Path.Combine(savedDir, name), overwrite: true);
        }
    }
